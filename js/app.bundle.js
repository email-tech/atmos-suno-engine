...
window.__ATMOS_BUILD__ = {"commit":"0260da5","date":"2026-08-19","src":"3c2a7b38"};
 ...
 * are called. */
const ROLE_REFERENTS = Object.freeze({
...
const INSTRUMENT_REFERENTS = Object.freeze([
...
const phraseText = (p) => (typeof p === 'string' ? p : String(p && p.t || ''));
...
function satisfies(ref, arr, hostSlots = []) {
...
function isLegalTail(phrase, arr, dim) {
...
function legalTails(pool, arr, dim) {
...
function pickTail(pool, arr, dim, rand) {
...
function guardTail(pool, arr, dim, selected) {
...
Object.assign(window.__ATMOS, { referentsIn, satisfies, isLegalTail, legalTails, pickTail, guardTail, ROLE_REFERENTS, INSTRUMENT_REFERENTS, phraseText, HOST_SLOTS });
})();
...
function deriveSeed(baseBuildSeed, label) {
...
function rngFor(baseBuildSeed, label) {
 ...
 * earlier decision. */
function stablePick(items, rand, weightOf = () => 1) {
...
Object.assign(window.__ATMOS, { deriveSeed, rngFor, stablePick, SEED_LABELS });
})();
 ...
 * Suno fact.
 * ------------------------------------------------------------------------- */
const {rngFor, stablePick, SEED_LABELS} = window.__ATMOS;

/* ---- §8.4 VOCAL-ROLE TAXONOMY ---------------------------------------- */
const VOCAL_ROLES = Object.freeze([
...
function classifyVocalRole(source) {
...
const TREATMENT_SUBTYPES = Object.freeze({
...
function eligibleTalkboxCarriers(cast) {
...
const VOCAL_PROSE = Object.freeze({
  vocoderAccentLayer: {
...
const VAGUE_GLITCH_RE = /\bglitchy\b/i;
...
function eligibleVocalTargets(vocalSources, treatment) {
  return (vocalSources || [])
...
function resolveVocalTreatmentFull(ctx, policy) {
...
Object.assign(window.__ATMOS, { classifyVocalRole, eligibleTalkboxCarriers, eligibleVocalTargets, resolveVocalTreatmentFull, VOCAL_ROLES, TREATMENT_SUBTYPES, DEFAULT_INTENSITY, VOCAL_PROSE, VAGUE_GLITCH_RE });
})();
 ...
 * ------------------------------------------------------------------------- */
const {rngFor, stablePick, SEED_LABELS} = window.__ATMOS;
const {resolveVocalTreatmentFull} = window.__ATMOS;
...
const EAR_CANDY_MODES      = Object.freeze(['off', 'subtle', 'balanced', 'active']);
const SPACE_MOVEMENT_MODES = Object.freeze(['off', 'auto', 'widthDepth', 'rhythmicMotion', 'modulatedMotion', 'filterEvolution']);
const VOCAL_TREATMENT_MODES = Object.freeze(['off', 'auto', 'vocoder', 'talkbox', 'vocalChops', 'stutter', 'glitch']);
...
/* ---- ORCHESTRATION (§18.4) ------------------------------------------- */
function resolveDetailSystem(input) {
...
function renderDetailClauses(resolved) {
  if (!resolved) return [];
  const all = [
...
function detailNoOpReasons(resolved) {
  if (!resolved) return [];
...
Object.assign(window.__ATMOS, { defaultDetailState, emptyResolvedDetail, buildDetailContext, resolveVocalTreatment, resolveSpaceMovement, resolveEarCandy, resolveDetailSystem, renderDetailClauses, detailNoOpReasons, EAR_CANDY_MODES, SPACE_MOVEMENT_MODES, VOCAL_TREATMENT_MODES });
})();
...
const SUSTAINING_RE = /\b(strings?|violin|violins|viola|violas|cello|cellos|contrabass|double bass|string section|string ensemble|brass|horn|horns|french horn|trumpet|trumpets|trombone|trombones|tuba|flute|flutes|piccolo|oboe|clarinet|clarinets|bassoon|cor anglais|ney|bansuri|shakuhachi|duduk|pan-flute|panpipes|recorder|saxophone|organ|harmonium|accordion|choir|chorus)\b/i;
...
const STRUCK_OR_PLUCKED_RE = /\b(harp|harps|marimba|vibraphone|vibes|glockenspiel|xylophone|celesta|celeste|timpani|tubular bells?|chimes?|crotales|mallets?|dulcimer|cimbalom|piano|harpsichord|pizzicato|guitar|lute|kora|koto|sitar|kalimba)\b/i;
...
const ATTACK_RE = /\b(stab|stabs|stabbing|staccato|spiccato|marcato|fanfare|ostinato|clipped|choppy|punchy|jabbing|short accented?)\b/i;

/* Wording that states a held note. */
const LEGATO_RE = /\b(legato|sustained?|sustains|held|holding|long|drone|swell|swells|swelling|bowed|sweeping|soaring|slow|singing|flowing|breathy|keening|wailing|plaintive|aching|glissando|melisma|pedal)\b/i;

const isSustaining = (t) => SUSTAINING_RE.test(String(t || '')) && !STRUCK_OR_PLUCKED_RE.test(String(t || ''));
const isStruckOrPlucked = (t) => STRUCK_OR_PLUCKED_RE.test(String(t || ''));
...
function articulationFault(text) {
  const t = String(text || '');
...
function planePhrase(plane, variant, subject) {
 ...
 * ------------------------------------------------------------------------*/
const GENRE_PROHIBITIONS = Object.freeze({
...
function violatesGenrePolicy(entry, genreText) {
  if (entry.priority === 'core' || entry.signature) return false;
...
const BED_BUDGET = 1;
const LEAD_BUDGET = 1;
const VOICE_BUDGET = null;   // DEFERRED_TEST — see above. Do not guess.
 ...
 * ------------------------------------------------------------------------*/
function buildCast(heldAtoms, opts) {
  const o = opts || {};
 ...
 * ------------------------------------------------------------------------*/
function reconcileCast(cast, opts) {
  const o = opts || {};
  const genre = o.genre || '';
...
Object.assign(window.__ATMOS, { componentsFor, isLead, isSustainedBed, wastesItsSlot, violatesGenrePolicy, buildCast, reconcileCast, COMPOSITE_COMPONENTS, GENRE_PROHIBITIONS, BED_BUDGET, LEAD_BUDGET, VOICE_BUDGET });
})();
...
const QUALITY_THRESHOLD = 85; // John, 2026-08-12 — approved pass mark
const MAX_LYRIC_ATTEMPTS = 3; // initial generation + up to 2 repairs
...
function parseLineLengthSpec(spec) {
...
function parseRhymeDensitySpec(spec) {
...
function checkSectionLabels(lyricsText, requiredLabels) {
...
function checkSyllables(lyricsText, lineLengthSpec) {
...
function checkRhymeDensity(lyricsText, rhymeDensitySpec) {
...
function checkDensity(lyricsText, energy) {
...
function validateLyrics(lyricsText, brief) {
...
Object.assign(window.__ATMOS, { countSyllables, lineSyllables, rhymeKey, wordsRhyme, parseLineLengthSpec, parseRhymeDensitySpec, parseLyricSections, checkSectionLabels, checkSyllables, checkRhymeDensity, checkDensity, validateLyrics, QUALITY_THRESHOLD, MAX_LYRIC_ATTEMPTS });
})();
...
function buildRepairPrompt(brief, initialResult, qualityResult) {
...
const P = {
  ...
  lead: {
    ...
    brassFanfare:    { t: 'a heroic long legato brass line', d: 'A' },
...
const P = {
  pads: {
    ...
    marimbaBed:     { t: 'a soft layered marimba figure', d: 'A' },
...
function unsupportedParams(provider, model) {
  const c = loadCache();
  return c[keyFor(provider, model)] || [];
}

function markUnsupported(provider, model, param) {
  const c = loadCache();
...
function clearUnsupportedParams() { cache = {}; saveCache(); }
...
const DROPPABLE_PARAMS = Object.freeze([
...
function parseUnsupportedParam(status, body) {
...
function omitUnsupported(params, provider, model) {
...
async function sendWithParamRetry({ provider, model, params, attempt }) {
...
Object.assign(window.__ATMOS, { sendWithParamRetry, unsupportedParams, markUnsupported, clearUnsupportedParams, parseUnsupportedParam, omitUnsupported, DROPPABLE_PARAMS, MAX_PARAM_RETRIES });
})();
...
const {sendWithParamRetry} = window.__ATMOS;
